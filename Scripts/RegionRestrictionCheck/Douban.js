...
!(async () => {
    const movieInfo = await getMovieCalendar()
    console.log('获取到的电影信息:')
    console.log(JSON.stringify(movieInfo, null, 2))
    
    console.log('详细电影信息:')
    
    await notify(movieInfo)
})()
...
async function getMovieCalendar() {
    ...
    try {
        ...
        const { comment, subject, today } = response
        const { rating, card_subtitle, url, title, id } = subject
        ...
        // 获取详细电影信息
        const detailedInfo = await getDetailedMovieInfo(id, apiKey)

        return {
            title: title,
            chineseTitle: title,
            originalTitle: detailedInfo.original_title || '',
            rating: rating.value,
...
async function getDetailedMovieInfo(movieId, apiKey) {
    const opts = {
        url: `https://frodo.douban.com/api/v2/movie/subject/${movieId}?apikey=${apiKey}&alt=json&_sig=tuOyn%2B2uZDBFGAFBLklc2GkuQk4%3D&_ts=1610703479`,
        method: 'GET',
        headers: {
            'User-Agent': 'api-client/0.1.3 com.douban.frodo/8.0.0'
        }
    ...
    try {
        const response = await Request(opts)
        console.log('详细电影信息原始响应:')
        console.log(JSON.stringify(response, null, 2))
        return response
    } catch (e) {
        console.log('获取详细电影信息失败:')
        console.log(e)
        return {}
    }
}
