...
!(async () => {
    ...
    $.log($.toStr(info))
    const { ip, type, asn, countryCode, country, city, state, trustScore, trustScoreRisk, organization, operator, analysis } = info
    ...
    let content = [
      `🌐 NETWORK IP: ${info.ip || '-'}`,
      `🅾️ AIGC IP: ${chatStatus.ip}`,
      `⛳️ COUNTRY/CITY: ${getFlagEmoji(info.countryCode)} ${info.city ? info.city : info.country}`,
      `⛩️ ORGANIZATION: ${info.organization}`,
      `💎 REPUTATION: ${info.type} | ${info.trustScore} - ${info.trustScoreRisk}`,
      `〽️ ANALYSIS: ${info.analysis}`,
...
async function getInfo() {
    ...
    if (ipv6) {
      ...
      if (ipv4) {
        ...
        // 使用正则表达式提取所需信息
        const ip = ipv4
        const type = body.match(/<div class="sidebar-card-title">\s*Type\s*<\/div>\s*<div>\s*([\w\s]+)\s*<\/div>/)?.[1]?.trim() || '-'
        ...
        // 新增：抓取 ANALYSIS 信息
        const analysis = body.match(/<div class="sidebar-summary-footer">\s*<div class="sidebar-card-title">\s*Analysis\s*<\/div>\s*([\s\S]*?)\s*<\/div>/)?.[1]?.trim() || '-'
      ...
      $.log('Analysis:', analysis)  // 新增：记录 Analysis 信息
    }
...
async function checkOpenAI() {
  ...
  try {
    ...
    const colo = traceInfo.colo

    const result = {
      status: `🅰🅸: ${status} ${emoji} ${colo}`,
      ip: ip
    }
