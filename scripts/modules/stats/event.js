...
async function getDirectInfo() {
  ...
  if (!CN_IP || !CN_ADDR) {  
    try {  
      ...
      const cnAddrMatch = body.match(/地址\s*(:|：)\s*(.*)/);
      const operatorMatch = body.match(/运营商\s*(:|：)\s*(.*)/);

      if (operatorMatch && cnAddrMatch) {
          CN_ADDR = `${cnAddrMatch[2].replace(/中国\s*/, '')} | ${operatorMatch[2].replace(/中国\s*/, '')}`;
      } else {
          // 如果没有找到运营商信息，从“数据二”提取信息
          const dataSecondLine = body.match(/数据二\s*(:|：)\s*(.*?)\n/);
          if (dataSecondLine) {
              CN_ADDR = dataSecondLine[2].replaceAll('|', '-'); // 替换所有的 '|' 为 '-'
          }      
      }
    } catch (e) {  
      $.logErr(e)   
...
async function getProxyInfo() {  
  ...
  try {  
    const res = await $.http.get({  
      ...
      headers: {
        'User-Agent': 'Mozilla/5.0 (...) Mobile/15E148 Safari/604.1 Edg/109.0.0.0',   
      },
    });  
    let body = JSON.parse($.lodash_get(res, 'body'));

    // 提取IP地址  
    PROXY_IP = $.lodash_get(body, 'ip', '-');  
    ...
    // 将组合的地址赋值给 PROXY_ADDR
    PROXY_ADDR = locationDetails;
  } catch (e) {  
    $.logErr(e);
  }
