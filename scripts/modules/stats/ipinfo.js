...
<<<<<<< HEAD
    const { clientIp = '-', city = '-', region = '-', country = '-', asn = '-', asOrganization = '-', colo = {} } = info
...
=======
    ...
    let content = [
      `🅖 & 🅰🅸: ${chatStatus.ip} & ${ip}`,
      `❶: ${flag} ${country} | ${city}, ${region}`, 
      `❷: ${info.asNumber} | ${company.name || '-'}`,
      `❸: ${verifiedStatus} | Test completed at ${new Date().toTimeString().split(' ')[0]}`
>>>>>>> 52c0162dedc816668a91d18d358120999b39b247
    ].join('\n')
...
<<<<<<< HEAD
    ...
    // 使用 Cloudflare Speed Test API 查询 IP 信息
    const res = await $.http.get({
